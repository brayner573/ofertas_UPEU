...
$password = $_POST['txt_password']; // Se usará para la comparación con el hash de la contraseña
    ...
    if (password_verify($password, $fila['contrasenia'])) {
        ...
        if ($fila['estado_asignacion'] == 0) {
            ...
            if ($_SESSION["SESION_ROL"] == 2) {
                // Si es empresa, cargar la interfaz directamente
                include("panel_empresa.php");
                exit();
            } else {
                // Otros roles (ej: postulante o administrador)
